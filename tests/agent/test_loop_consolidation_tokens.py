...
async def test_fast_trim_noop_when_under_threshold(tmp_path) -> None:
    ...
    result = loop.memory_consolidator.fast_trim_if_needed(session)
...
async def test_fast_trim_advances_last_consolidated(tmp_path, monkeypatch) -> None:
    ...
    chunk = loop.memory_consolidator.fast_trim_if_needed(session)
...
async def test_archive_trimmed_chunk_holds_lock(tmp_path) -> None:
    ...
    async def mock_consolidate(messages):
        ...
        return True
    loop.memory_consolidator.consolidate_messages = mock_consolidate  # type: ignore[method-assign]

    chunk = [{"role": "user", "content": "u1"}, {"role": "assistant", "content": "a1"}]
    await loop.memory_consolidator.archive_trimmed_chunk("cli:test", chunk)

    assert lock_held_during_consolidate[0], "consolidate_messages must run under per-session lock"
